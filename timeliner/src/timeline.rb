  ...
  results = {
    ...
    items: {
      points: {
        ...
        unknown_service_group_events: unknown_service_events.flat_map { |cid, events| events[:events].flat_map {|event| { id: cid + " " + event[:action] + " " + event[:timestamp].to_s, action: event[:action], groupId: cid, start: event[:timestamp], ext_code: event[:ext_code], statuses: get_status_event(event), src_jsons: event[:src_jsons].to_json } } }
      },
      ranges: {
